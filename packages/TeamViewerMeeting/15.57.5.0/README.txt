...
⚠️  Let op: EXE installer bestand is NIET meegeleverd (meestal te groot voor Git).
...
2. Download de EXE installer: TeamViewerMeeting_Setup_x64.exe
3. Maak een folder structuur:
   ...
   │   └── Files/
   │       └── TeamViewerMeeting_Setup_x64.exe      (jouw EXE bestand)
...
Path:              C:\Program Files\TeamViewer\TeamViewerMeeting
File or folder:    TeamViewerMeeting.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor TeamViewerMeeting
$minVersion = [version]"15.57.5.0"
...
foreach ($path in $regPaths) {
    $app = Get-ItemProperty -Path $path -ErrorAction SilentlyContinue | 
           Where-Object { $_.DisplayName -like "*TeamViewerMeeting*" }
    
    if ($app) {
        try {
            $installedVersion = [version]$app.DisplayVersion
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: TeamViewerMeeting $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: TeamViewerMeeting"
            exit 0
        }
    }
}
...
$filePaths = @(
    "C:\Program Files\TeamViewer\TeamViewerMeeting\TeamViewerMeeting.exe",
    "C:\Program Files (x86)\TeamViewer\TeamViewerMeeting\TeamViewerMeeting.exe"
)
...
2. Verify detection na installatie:
   ...
   # File check
   Test-Path "C:\Program Files\TeamViewer\TeamViewerMeeting\TeamViewerMeeting.exe"
