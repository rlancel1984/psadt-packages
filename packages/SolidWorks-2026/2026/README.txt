...
2. Download de EXE installer: SolidWorksSetup.exe
   
   📥 Download locaties (kies één):
      • Vendor website: Zoek "SolidWorks SolidWorks 2026 2026"
...
3. Maak een folder structuur:
   ...
   │   └── Files/
   │       └── SolidWorksSetup.exe      ⚠️ HANDMATIG TOEVOEGEN
...
Path:              C:\Program Files\SolidWorks\SolidWorks 2026
File or folder:    SolidWorks 2026.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor SolidWorks 2026
...
foreach ($path in $regPaths) {
    $app = Get-ItemProperty -Path $path -ErrorAction SilentlyContinue | 
           Where-Object { $_.DisplayName -like "*SolidWorks 2026*" }
    
    if ($app) {
        try {
            $installedVersion = [version]$app.DisplayVersion
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: SolidWorks 2026 $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: SolidWorks 2026"
            exit 0
        }
    }
}
...
$filePaths = @(
    "C:\Program Files\SolidWorks\SolidWorks 2026\SolidWorks 2026.exe",
    "C:\Program Files (x86)\SolidWorks\SolidWorks 2026\SolidWorks 2026.exe"
)
...
2. Verify detection na installatie:
   ...
   # File check
   Test-Path "C:\Program Files\SolidWorks\SolidWorks 2026\SolidWorks 2026.exe"
