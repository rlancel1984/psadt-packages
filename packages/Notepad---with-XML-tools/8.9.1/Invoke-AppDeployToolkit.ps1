...
$adtSession = @{
    # App variables.
    AppVendor = 'Notepad'
    AppName = 'Notepad++ with XML tools'
    AppVersion = '8.9.1'
    AppArch = 'x64'
    AppLang = 'EN'
    AppRevision = '01'
...
function Install-ADTDeployment
{
    ...
    # Plugin: xmltools-master.zip
    $pluginSource = "$($adtSession.DirFiles)\xmltools-master.zip"
    $pluginTarget = "$env:ProgramFiles\Notepad++\plugins\xmltools-master"
    ...
    # Extract ZIP plugin
    try {
        Write-ADTLogEntry -Message "Extracting plugin: ${pluginSource} to ${pluginTarget}" -Severity 1
        Expand-Archive -Path $pluginSource -DestinationPath $pluginTarget -Force
