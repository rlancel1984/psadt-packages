...
Path:              C:\Program Files\Adobe Inc.\Adobe Acrobat Reader
File or folder:    Adobe Acrobat Reader.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor Adobe Acrobat Reader
    ...
    if ($app) {
        try {
            ...
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: Adobe Acrobat Reader $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: Adobe Acrobat Reader (version check failed)"
            exit 0
        }
    }
}
...
Task Name:       WingetAutoUpdate_AdobeAcrobatReader
...
Update logs:     %TEMP%\winget_update_AdobeAcrobatReader.log
PSADT logs:      C:\Windows\Logs\Software\Adobe Acrobat Reader\*.log

View scheduled task:
  • Open: Task Scheduler (taskschd.msc)
  • Navigeer: Task Scheduler Library
  • Zoek: WingetAutoUpdate_AdobeAcrobatReader
  • Rechtermuisknop: Run (om handmatig te testen)

View last run result:
  Get-ScheduledTask -TaskName "WingetAutoUpdate_AdobeAcrobatReader" | 
