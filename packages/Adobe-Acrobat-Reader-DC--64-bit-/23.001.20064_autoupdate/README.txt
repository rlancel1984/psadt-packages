...
║   Adobe Acrobat Reader DC (64-bit) 23.001.20064                                             
║                                                                        ║
╚════════════════════════════════════════════════════════════════════════╝
...
Rule type:         Registry
Key path:          HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Adobe.Acrobat.Reader.64-bit
Value name:        DisplayName
Detection method:  String comparison
Operator:          Equals
Value:             Adobe Acrobat Reader DC (64-bit)

Alternative registry path (probeer beide):
HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Adobe.Acrobat.Reader.64-bit
...
Path:              C:\Program Files\Adobe\Adobe Acrobat Reader DC (64-bit)
File or folder:    Adobe Acrobat Reader DC (64-bit).exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor Adobe Acrobat Reader DC (64-bit)
$minVersion = [version]"23.001.20064"

# Check via Registry
$regPaths = @(
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Adobe.Acrobat.Reader.64-bit",
    "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Adobe.Acrobat.Reader.64-bit"
)

foreach ($path in $regPaths) {
    $app = Get-ItemProperty -Path $path -ErrorAction SilentlyContinue | 
           Where-Object { $_.DisplayName -like "*Adobe Acrobat Reader DC (64-bit)*" }
    
    if ($app) {
        try {
            $installedVersion = [version]$app.DisplayVersion
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: Adobe Acrobat Reader DC (64-bit) $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: Adobe Acrobat Reader DC (64-bit) (version check failed)"
            exit 0
        }
    }
}
...
Test handmatig met:
winget search Adobe.Acrobat.Reader.64-bit
...
Task Name:       WingetAutoUpdate_AdobeAcrobatReaderDC64bit
Trigger:         Dagelijks om 03:00 uur
Account:         SYSTEM (LocalSystem)
Privileges:      Hoogste niveau (Administrator)

Command:         winget upgrade --id Adobe.Acrobat.Reader.64-bit --silent --scope machine
...
Update logs:     %TEMP%\winget_update_AdobeAcrobatReaderDC64bit.log
PSADT logs:      C:\Windows\Logs\Software\Adobe Acrobat Reader DC (64-bit)\*.log

View scheduled task:
  • Open: Task Scheduler (taskschd.msc)
  • Navigeer: Task Scheduler Library
  • Zoek: WingetAutoUpdate_AdobeAcrobatReaderDC64bit
  • Rechtermuisknop: Run (om handmatig te testen)

View last run result:
  Get-ScheduledTask -TaskName "WingetAutoUpdate_AdobeAcrobatReaderDC64bit" | 
    Get-ScheduledTaskInfo | 
    Select LastRunTime, LastTaskResult

Manual update test:
  winget upgrade --id Adobe.Acrobat.Reader.64-bit --silent --scope machine
