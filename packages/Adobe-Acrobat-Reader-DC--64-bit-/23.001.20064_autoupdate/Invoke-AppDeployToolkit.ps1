...
$adtSession = @{
    # App variables.
    AppVendor = 'Adobe'
    AppName = 'Adobe Acrobat Reader DC (64-bit)'
...
function Install-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Creating scheduled task for automatic updates: WingetAutoUpdate_AdobeAcrobatReaderDC64bit" -Severity 1
    
    try {
        # Task Action: Run winget upgrade
        $action = New-ScheduledTaskAction `
            -Execute "powershell.exe" `
            -Argument "-NoProfile -WindowStyle Hidden -Command `"`& { winget upgrade --id Adobe.Acrobat.Reader.64-bit --silent --accept-package-agreements --accept-source-agreements | Out-File -FilePath `$env:TEMP\winget_update_AdobeAcrobatReaderDC64bit.log -Append }`"`""
        ...
        Register-ScheduledTask `
            -TaskName "WingetAutoUpdate_AdobeAcrobatReaderDC64bit" `
            -Action $action `
            -Trigger $trigger `
            -Principal $principal `
            -Settings $settings `
            -Description "Automatic update for Adobe Acrobat Reader DC (64-bit) via Winget. Runs daily at 3:00 AM or at next device startup if missed." `
            -Force | Out-Null
        
        Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_AdobeAcrobatReaderDC64bit' created successfully" -Severity 1
    }
    catch {
...
function Uninstall-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Removing scheduled task: WingetAutoUpdate_AdobeAcrobatReaderDC64bit" -Severity 1
    
    try {
        $task = Get-ScheduledTask -TaskName "WingetAutoUpdate_AdobeAcrobatReaderDC64bit" -ErrorAction SilentlyContinue
        if ($task) {
            Unregister-ScheduledTask -TaskName "WingetAutoUpdate_AdobeAcrobatReaderDC64bit" -Confirm:$false
            Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_AdobeAcrobatReaderDC64bit' removed successfully" -Severity 1
        } else {
            Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_AdobeAcrobatReaderDC64bit' not found (already removed or never created)" -Severity 2
        }
    }
    catch {
