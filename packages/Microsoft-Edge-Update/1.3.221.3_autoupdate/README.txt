...
║   Microsoft Edge Update 1.3.221.3                                             
║                                                                        ║
╚════════════════════════════════════════════════════════════════════════╝
...
└── Toolkit/Files/MicrosoftEdgeWebview2Setup.exe   ← EXE installer (INCLUDED)
...
✅ De EXE installer (MicrosoftEdgeWebview2Setup.exe) is AL INCLUDED in deze package!
...
2. Alle bestanden zijn aanwezig - geen extra downloads nodig
   ...
   │   └── Files/
   │       └── MicrosoftEdgeWebview2Setup.exe      ✅ INCLUDED
...
• HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft Corporation\Microsoft Edge Update
...
Path:              C:\Program Files\Microsoft Corporation\Microsoft Edge Update
File or folder:    Microsoft Edge Update.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor Microsoft Edge Update
$minVersion = [version]"1.3.221.3"
...
foreach ($path in $regPaths) {
    $app = Get-ItemProperty -Path $path -ErrorAction SilentlyContinue | 
           Where-Object { $_.DisplayName -like "*Microsoft Edge Update*" }
    
    if ($app) {
        try {
            $installedVersion = [version]$app.DisplayVersion
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: Microsoft Edge Update $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: Microsoft Edge Update"
            exit 0
        }
    }
}
...
$filePaths = @(
    "C:\Program Files\Microsoft Corporation\Microsoft Edge Update\Microsoft Edge Update.exe",
    "C:\Program Files (x86)\Microsoft Corporation\Microsoft Edge Update\Microsoft Edge Update.exe"
)
...
WiX installer gedetecteerd.
Silent parameters: /quiet /norestart

Deze installer is gebaseerd op Windows Installer (MSI).
...
2. Verify detection na installatie:
   ...
   # File check
   Test-Path "C:\Program Files\Microsoft Corporation\Microsoft Edge Update\Microsoft Edge Update.exe"
