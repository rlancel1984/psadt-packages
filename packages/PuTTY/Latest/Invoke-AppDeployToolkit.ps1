...
function Install-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Installing PuTTY.PuTTY via Winget..." -Severity 1
    $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'install --id PuTTY.PuTTY --silent --accept-source-agreements --accept-package-agreements --scope machine' -PassThru
    ...
    Write-ADTLogEntry -Message "Creating scheduled task for automatic updates: WingetAutoUpdate_PuTTY" -Severity 1
    
    try {
        # Task Action: Run winget upgrade
        $action = New-ScheduledTaskAction `
            -Execute "powershell.exe" `
            -Argument "-NoProfile -WindowStyle Hidden -Command `"`& { winget upgrade --id PuTTY.PuTTY --silent --accept-package-agreements --accept-source-agreements --scope machine | Out-File -FilePath `$env:TEMP\winget_update_PuTTY.log -Append }`"`""
        ...
        Register-ScheduledTask `
            -TaskName "WingetAutoUpdate_PuTTY" `
            -Action $action `
            -Trigger $trigger `
            -Principal $principal `
            -Settings $settings `
            -Description "Automatic update for PuTTY via Winget. Runs daily at 3:00 AM or at next device startup if missed." `
            -Force | Out-Null
        
        Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_PuTTY' created successfully" -Severity 1
    }
    catch {
...
function Uninstall-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Uninstalling PuTTY.PuTTY via Winget..." -Severity 1
    $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'uninstall --id PuTTY.PuTTY --silent --scope machine' -PassThru
    ...
    Write-ADTLogEntry -Message "Removing scheduled task: WingetAutoUpdate_PuTTY" -Severity 1
    
    try {
        $task = Get-ScheduledTask -TaskName "WingetAutoUpdate_PuTTY" -ErrorAction SilentlyContinue
        if ($task) {
            Unregister-ScheduledTask -TaskName "WingetAutoUpdate_PuTTY" -Confirm:$false
            Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_PuTTY' removed successfully" -Severity 1
        } else {
            Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_PuTTY' not found (already removed or never created)" -Severity 2
        }
    }
    catch {
