...
└── Toolkit/Files/IronKey.exe   ← EXE installer (INCLUDED)
...
✅ De EXE installer (IronKey.exe) is AL INCLUDED in deze package!
...
• HKEY_LOCAL_MACHINE\SOFTWARE\Kingston\IronKey
...
Path:              C:\Program Files\Kingston\IronKey
File or folder:    IronKey.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor IronKey
$minVersion = [version]"3.5.1.0"
...
foreach ($path in $regPaths) {
    $app = Get-ItemProperty -Path $path -ErrorAction SilentlyContinue | 
           Where-Object { $_.DisplayName -like "*IronKey*" }
    
    if ($app) {
        try {
            $installedVersion = [version]$app.DisplayVersion
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: IronKey $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: IronKey"
            exit 0
        }
    }
}
...
$filePaths = @(
    "C:\Program Files\Kingston\IronKey\IronKey.exe",
    "C:\Program Files (x86)\Kingston\IronKey\IronKey.exe"
)
...
2. Verify detection na installatie:
   ...
   # File check
   Test-Path "C:\Program Files\Kingston\IronKey\IronKey.exe"
