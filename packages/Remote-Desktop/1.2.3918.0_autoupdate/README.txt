...
Rule type:         Registry
Key path:          HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft.RemoteDesktopClient
...
HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft.RemoteDesktopClient
...
Path:              C:\Program Files\Microsoft Corporation\Remote Desktop
File or folder:    Remote Desktop.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor Remote Desktop
$minVersion = [version]"1.2.3918.0"

# Check via Registry
$regPaths = @(
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft.RemoteDesktopClient",
    "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft.RemoteDesktopClient"
)
    ...
    if ($app) {
        try {
            ...
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: Remote Desktop $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: Remote Desktop (version check failed)"
            exit 0
        }
    }
}
...
Package ID:  Microsoft.RemoteDesktopClient
Source:      winget (Microsoft repository)

Test handmatig met:
winget search Microsoft.RemoteDesktopClient
winget show Microsoft.RemoteDesktopClient
...
Task Name:       WingetAutoUpdate_RemoteDesktop
Trigger:         Dagelijks om 03:00 uur
Account:         SYSTEM (LocalSystem)
Privileges:      Hoogste niveau (Administrator)

Command:         winget upgrade --id Microsoft.RemoteDesktopClient --silent --scope machine
...
Update logs:     %TEMP%\winget_update_RemoteDesktop.log
PSADT logs:      C:\Windows\Logs\Software\Remote Desktop\*.log

View scheduled task:
  • Open: Task Scheduler (taskschd.msc)
  • Navigeer: Task Scheduler Library
  • Zoek: WingetAutoUpdate_RemoteDesktop
  • Rechtermuisknop: Run (om handmatig te testen)

View last run result:
  Get-ScheduledTask -TaskName "WingetAutoUpdate_RemoteDesktop" | 
    Get-ScheduledTaskInfo | 
    Select LastRunTime, LastTaskResult

Manual update test:
  winget upgrade --id Microsoft.RemoteDesktopClient --silent --scope machine
