...
function Install-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Installing Microsoft.RemoteDesktopClient via Winget..." -Severity 1
    ...
    $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'install --id Microsoft.RemoteDesktopClient --silent --accept-source-agreements --accept-package-agreements' -PassThru -IgnoreExitCodes @(0,-1978335135)
    ...
    if ($wingetResult.ExitCode -eq -1978335135) {
        ...
        $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'upgrade --id Microsoft.RemoteDesktopClient --silent --accept-source-agreements --accept-package-agreements' -PassThru -IgnoreExitCodes @(0,-1978335212)
    ...
    Write-ADTLogEntry -Message "Creating scheduled task for automatic updates: WingetAutoUpdate_RemoteDesktop" -Severity 1
    
    try {
        # Task Action: Run winget upgrade
        $action = New-ScheduledTaskAction `
            -Execute "powershell.exe" `
            -Argument "-NoProfile -WindowStyle Hidden -Command `"`& { winget upgrade --id Microsoft.RemoteDesktopClient --silent --accept-package-agreements --accept-source-agreements | Out-File -FilePath `$env:TEMP\winget_update_RemoteDesktop.log -Append }`"`""
        ...
        Register-ScheduledTask `
            -TaskName "WingetAutoUpdate_RemoteDesktop" `
            -Action $action `
            -Trigger $trigger `
            -Principal $principal `
            -Settings $settings `
            -Description "Automatic update for Remote Desktop via Winget. Runs daily at 3:00 AM or at next device startup if missed." `
            -Force | Out-Null
        
        Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_RemoteDesktop' created successfully" -Severity 1
    }
    catch {
...
function Uninstall-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Uninstalling Microsoft.RemoteDesktopClient via Winget..." -Severity 1
    
    # Uninstall without scope (most compatible)
    $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'uninstall --id Microsoft.RemoteDesktopClient --silent --accept-source-agreements' -PassThru
    ...
    Write-ADTLogEntry -Message "Removing scheduled task: WingetAutoUpdate_RemoteDesktop" -Severity 1
    
    try {
        $task = Get-ScheduledTask -TaskName "WingetAutoUpdate_RemoteDesktop" -ErrorAction SilentlyContinue
        if ($task) {
            Unregister-ScheduledTask -TaskName "WingetAutoUpdate_RemoteDesktop" -Confirm:$false
            Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_RemoteDesktop' removed successfully" -Severity 1
        } else {
            Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_RemoteDesktop' not found (already removed or never created)" -Severity 2
        }
    }
    catch {
