...
Rule type:         Registry
Key path:          HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Oracle.JavaRuntimeEnvironment
...
HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Oracle.JavaRuntimeEnvironment
...
Path:              C:\Program Files\Oracle\Java Runtime
File or folder:    Java Runtime.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor Java Runtime
$minVersion = [version]"Latest"

# Check via Registry
$regPaths = @(
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Oracle.JavaRuntimeEnvironment",
    "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Oracle.JavaRuntimeEnvironment"
)

foreach ($path in $regPaths) {
    $app = Get-ItemProperty -Path $path -ErrorAction SilentlyContinue | 
           Where-Object { $_.DisplayName -like "*Java Runtime*" }
    
    if ($app) {
        try {
            $installedVersion = [version]$app.DisplayVersion
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: Java Runtime $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: Java Runtime (version check failed)"
            exit 0
        }
    }
}
...
Package ID:  Oracle.JavaRuntimeEnvironment
Source:      winget (Microsoft repository)

Test handmatig met:
winget search Oracle.JavaRuntimeEnvironment
winget show Oracle.JavaRuntimeEnvironment
...
Task Name:       WingetAutoUpdate_JavaRuntime
Trigger:         Dagelijks om 03:00 uur
Account:         SYSTEM (LocalSystem)
Privileges:      Hoogste niveau (Administrator)

Command:         winget upgrade --id Oracle.JavaRuntimeEnvironment --silent --scope machine
...
Update logs:     %TEMP%\winget_update_JavaRuntime.log
PSADT logs:      C:\Windows\Logs\Software\Java Runtime\*.log

View scheduled task:
  • Open: Task Scheduler (taskschd.msc)
  • Navigeer: Task Scheduler Library
  • Zoek: WingetAutoUpdate_JavaRuntime
  • Rechtermuisknop: Run (om handmatig te testen)

View last run result:
  Get-ScheduledTask -TaskName "WingetAutoUpdate_JavaRuntime" | 
    Get-ScheduledTaskInfo | 
    Select LastRunTime, LastTaskResult

Manual update test:
  winget upgrade --id Oracle.JavaRuntimeEnvironment --silent --scope machine
