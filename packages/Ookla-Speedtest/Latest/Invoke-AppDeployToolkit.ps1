...
function Install-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Installing Ookla.Speedtest.Desktop via Winget..." -Severity 1
    $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'install --id Ookla.Speedtest.Desktop --silent --accept-source-agreements --accept-package-agreements --scope machine' -PassThru
    ...
    Write-ADTLogEntry -Message "Creating scheduled task for automatic updates: WingetAutoUpdate_OoklaSpeedtest" -Severity 1
    
    try {
        # Task Action: Run winget upgrade
        $action = New-ScheduledTaskAction `
            -Execute "powershell.exe" `
            -Argument "-NoProfile -WindowStyle Hidden -Command `"`& { winget upgrade --id Ookla.Speedtest.Desktop --silent --accept-package-agreements --accept-source-agreements --scope machine | Out-File -FilePath `$env:TEMP\winget_update_OoklaSpeedtest.log -Append }`"`""
        ...
        Register-ScheduledTask `
            -TaskName "WingetAutoUpdate_OoklaSpeedtest" `
            -Action $action `
            -Trigger $trigger `
            -Principal $principal `
            -Settings $settings `
            -Description "Automatic update for Ookla Speedtest via Winget. Runs daily at 3:00 AM or at next device startup if missed." `
            -Force | Out-Null
        
        Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_OoklaSpeedtest' created successfully" -Severity 1
    }
    catch {
...
function Uninstall-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Uninstalling Ookla.Speedtest.Desktop via Winget..." -Severity 1
    $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'uninstall --id Ookla.Speedtest.Desktop --silent --scope machine' -PassThru
    ...
    Write-ADTLogEntry -Message "Removing scheduled task: WingetAutoUpdate_OoklaSpeedtest" -Severity 1
    
    try {
        $task = Get-ScheduledTask -TaskName "WingetAutoUpdate_OoklaSpeedtest" -ErrorAction SilentlyContinue
        if ($task) {
            Unregister-ScheduledTask -TaskName "WingetAutoUpdate_OoklaSpeedtest" -Confirm:$false
            Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_OoklaSpeedtest' removed successfully" -Severity 1
        } else {
            Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_OoklaSpeedtest' not found (already removed or never created)" -Severity 2
        }
    }
    catch {
