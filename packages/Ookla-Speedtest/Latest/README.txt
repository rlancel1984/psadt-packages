...
Rule type:         Registry
Key path:          HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Ookla.Speedtest.Desktop
Value name:        DisplayName
Detection method:  String comparison
Operator:          Equals
Value:             Ookla Speedtest

Alternative registry path (probeer beide):
HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Ookla.Speedtest.Desktop
...
Path:              C:\Program Files\Ookla\Ookla Speedtest
File or folder:    Ookla Speedtest.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor Ookla Speedtest
$minVersion = [version]"Latest"

# Check via Registry
$regPaths = @(
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Ookla.Speedtest.Desktop",
    "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Ookla.Speedtest.Desktop"
)

foreach ($path in $regPaths) {
    $app = Get-ItemProperty -Path $path -ErrorAction SilentlyContinue | 
           Where-Object { $_.DisplayName -like "*Ookla Speedtest*" }
    
    if ($app) {
        try {
            $installedVersion = [version]$app.DisplayVersion
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: Ookla Speedtest $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: Ookla Speedtest (version check failed)"
            exit 0
        }
    }
}
...
Package ID:  Ookla.Speedtest.Desktop
Source:      winget (Microsoft repository)

Test handmatig met:
winget search Ookla.Speedtest.Desktop
winget show Ookla.Speedtest.Desktop
...
Task Name:       WingetAutoUpdate_OoklaSpeedtest
Trigger:         Dagelijks om 03:00 uur
Account:         SYSTEM (LocalSystem)
Privileges:      Hoogste niveau (Administrator)

Command:         winget upgrade --id Ookla.Speedtest.Desktop --silent --scope machine
...
Update logs:     %TEMP%\winget_update_OoklaSpeedtest.log
PSADT logs:      C:\Windows\Logs\Software\Ookla Speedtest\*.log

View scheduled task:
  • Open: Task Scheduler (taskschd.msc)
  • Navigeer: Task Scheduler Library
  • Zoek: WingetAutoUpdate_OoklaSpeedtest
  • Rechtermuisknop: Run (om handmatig te testen)

View last run result:
  Get-ScheduledTask -TaskName "WingetAutoUpdate_OoklaSpeedtest" | 
    Get-ScheduledTaskInfo | 
    Select LastRunTime, LastTaskResult

Manual update test:
  winget upgrade --id Ookla.Speedtest.Desktop --silent --scope machine
