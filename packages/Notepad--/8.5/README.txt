...
Rule type:         Registry
Key path:          HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Notepad++.Notepad++
...
HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Notepad++.Notepad++
...
Path:              C:\Program Files\Notepad++ Team\Notepad++
...
────────────────────────────────────────────────────────────────────────
# Detection script voor Notepad++
$minVersion = [version]"8.5"

# Check via Registry
$regPaths = @(
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Notepad++.Notepad++",
    "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Notepad++.Notepad++"
)
    ...
    if ($app) {
        try {
            ...
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: Notepad++ $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: Notepad++ (version check failed)"
            exit 0
        }
    }
}
...
Task Name:       WingetAutoUpdate_Notepad
Trigger:         Dagelijks om 03:00 uur
Account:         SYSTEM (LocalSystem)
Privileges:      Hoogste niveau (Administrator)

Command:         winget upgrade --id Notepad++.Notepad++ --silent --scope machine
...
Update logs:     %TEMP%\winget_update_Notepad.log
PSADT logs:      C:\Windows\Logs\Software\Notepad++\*.log

View scheduled task:
  • Open: Task Scheduler (taskschd.msc)
  • Navigeer: Task Scheduler Library
  • Zoek: WingetAutoUpdate_Notepad
  • Rechtermuisknop: Run (om handmatig te testen)

View last run result:
  Get-ScheduledTask -TaskName "WingetAutoUpdate_Notepad" | 
    Get-ScheduledTaskInfo | 
    Select LastRunTime, LastTaskResult

Manual update test:
  winget upgrade --id Notepad++.Notepad++ --silent --scope machine
