...
function Install-ADTDeployment
{
    ...
    # Install via Winget (with machine/user scope fallback)
    ...
    # Try machine scope first (preferred for enterprise deployments)
    Write-ADTLogEntry -Message "Attempting machine-scope installation..." -Severity 1
    $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'install --id Notepad++.Notepad++ --silent --accept-source-agreements --accept-package-agreements --scope machine' -PassThru -IgnoreExitCodes @(0,-1978335189)
    
    # Exit code -1978335189 (0x8A15002B) means machine scope not supported
    if ($wingetResult.ExitCode -eq -1978335189) {
        Write-ADTLogEntry -Message "Machine scope not supported by this package, falling back to user scope..." -Severity 2
