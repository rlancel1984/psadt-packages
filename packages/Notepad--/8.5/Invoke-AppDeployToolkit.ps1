...
function Install-ADTDeployment
{
    ...
    # Check if package is already installed (use -like instead of -match to avoid regex issues)
    $listOutput = & $wingetPath list --id Notepad++.Notepad++ --accept-source-agreements 2>&1
    $isInstalled = $listOutput -like '*Notepad++.Notepad++*'
    ...
    if ($isInstalled) {
        ...
        # Fresh install - try without scope first (most compatible)
