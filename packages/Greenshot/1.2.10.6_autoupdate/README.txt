...
Rule type:         Registry
Key path:          HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Greenshot.Greenshot
...
HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Greenshot.Greenshot
...
Path:              C:\Program Files\Greenshot\Greenshot
...
────────────────────────────────────────────────────────────────────────
# Detection script voor Greenshot
$minVersion = [version]"1.2.10.6"

# Check via Registry
$regPaths = @(
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Greenshot.Greenshot",
    "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Greenshot.Greenshot"
)
    ...
    if ($app) {
        try {
            ...
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: Greenshot $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: Greenshot (version check failed)"
            exit 0
        }
    }
}
...
Package ID:  Greenshot.Greenshot
Source:      winget (Microsoft repository)

Test handmatig met:
winget search Greenshot.Greenshot
winget show Greenshot.Greenshot
...
Task Name:       WingetAutoUpdate_Greenshot
Trigger:         Dagelijks om 03:00 uur
Account:         SYSTEM (LocalSystem)
Privileges:      Hoogste niveau (Administrator)

Command:         winget upgrade --id Greenshot.Greenshot --silent --scope machine
...
Update logs:     %TEMP%\winget_update_Greenshot.log
PSADT logs:      C:\Windows\Logs\Software\Greenshot\*.log

View scheduled task:
  • Open: Task Scheduler (taskschd.msc)
  • Navigeer: Task Scheduler Library
  • Zoek: WingetAutoUpdate_Greenshot
  • Rechtermuisknop: Run (om handmatig te testen)

View last run result:
  Get-ScheduledTask -TaskName "WingetAutoUpdate_Greenshot" | 
    Get-ScheduledTaskInfo | 
    Select LastRunTime, LastTaskResult

Manual update test:
  winget upgrade --id Greenshot.Greenshot --silent --scope machine
