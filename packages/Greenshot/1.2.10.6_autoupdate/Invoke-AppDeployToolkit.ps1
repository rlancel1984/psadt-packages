...
function Install-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Installing Greenshot.Greenshot via Winget..." -Severity 1
    ...
    $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'install --id Greenshot.Greenshot --silent --accept-source-agreements --accept-package-agreements' -PassThru -IgnoreExitCodes @(0,-1978335135)
    ...
    if ($wingetResult.ExitCode -eq -1978335135) {
        ...
        $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'upgrade --id Greenshot.Greenshot --silent --accept-source-agreements --accept-package-agreements' -PassThru -IgnoreExitCodes @(0,-1978335212)
    ...
    Write-ADTLogEntry -Message "Creating scheduled task for automatic updates: WingetAutoUpdate_Greenshot" -Severity 1
    
    try {
        # Task Action: Run winget upgrade
        $action = New-ScheduledTaskAction `
            -Execute "powershell.exe" `
            -Argument "-NoProfile -WindowStyle Hidden -Command `"`& { winget upgrade --id Greenshot.Greenshot --silent --accept-package-agreements --accept-source-agreements | Out-File -FilePath `$env:TEMP\winget_update_Greenshot.log -Append }`"`""
        ...
        Register-ScheduledTask `
            -TaskName "WingetAutoUpdate_Greenshot" `
            -Action $action `
            -Trigger $trigger `
            -Principal $principal `
            -Settings $settings `
            -Description "Automatic update for Greenshot via Winget. Runs daily at 3:00 AM or at next device startup if missed." `
            -Force | Out-Null
        
        Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_Greenshot' created successfully" -Severity 1
    }
    catch {
...
function Uninstall-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Uninstalling Greenshot.Greenshot via Winget..." -Severity 1
    
    # Uninstall without scope (most compatible)
    $wingetResult = Start-ADTProcess -FilePath $wingetPath -ArgumentList 'uninstall --id Greenshot.Greenshot --silent --accept-source-agreements' -PassThru
    ...
    Write-ADTLogEntry -Message "Removing scheduled task: WingetAutoUpdate_Greenshot" -Severity 1
    
    try {
        $task = Get-ScheduledTask -TaskName "WingetAutoUpdate_Greenshot" -ErrorAction SilentlyContinue
        if ($task) {
            Unregister-ScheduledTask -TaskName "WingetAutoUpdate_Greenshot" -Confirm:$false
            Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_Greenshot' removed successfully" -Severity 1
        } else {
            Write-ADTLogEntry -Message "Scheduled task 'WingetAutoUpdate_Greenshot' not found (already removed or never created)" -Severity 2
        }
    }
    catch {
