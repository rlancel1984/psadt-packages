...
║   Microsoft Azure Storage Explorer 1.28.1                                             
║                                                                        ║
╚════════════════════════════════════════════════════════════════════════╝
...
Rule type:         Registry
Key path:          HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft.Azure.StorageExplorer
Value name:        DisplayName
Detection method:  String comparison
Operator:          Equals
Value:             Microsoft Azure Storage Explorer

Alternative registry path (probeer beide):
HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft.Azure.StorageExplorer
...
Path:              C:\Program Files\Microsoft Corporation\Microsoft Azure Storage Explorer
File or folder:    Microsoft Azure Storage Explorer.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor Microsoft Azure Storage Explorer
$minVersion = [version]"1.28.1"

# Check via Registry
$regPaths = @(
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft.Azure.StorageExplorer",
    "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft.Azure.StorageExplorer"
)

foreach ($path in $regPaths) {
    $app = Get-ItemProperty -Path $path -ErrorAction SilentlyContinue | 
           Where-Object { $_.DisplayName -like "*Microsoft Azure Storage Explorer*" }
    
    if ($app) {
        try {
            $installedVersion = [version]$app.DisplayVersion
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: Microsoft Azure Storage Explorer $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: Microsoft Azure Storage Explorer (version check failed)"
            exit 0
        }
    }
}
...
Package ID:  Microsoft.Azure.StorageExplorer
Source:      winget (Microsoft repository)

Test handmatig met:
winget search Microsoft.Azure.StorageExplorer
winget show Microsoft.Azure.StorageExplorer
...
Task Name:       WingetAutoUpdate_MicrosoftAzureStorageExplorer
Trigger:         Dagelijks om 03:00 uur
Account:         SYSTEM (LocalSystem)
Privileges:      Hoogste niveau (Administrator)

Command:         winget upgrade --id Microsoft.Azure.StorageExplorer --silent --scope machine
...
Update logs:     %TEMP%\winget_update_MicrosoftAzureStorageExplorer.log
PSADT logs:      C:\Windows\Logs\Software\Microsoft Azure Storage Explorer\*.log

View scheduled task:
  • Open: Task Scheduler (taskschd.msc)
  • Navigeer: Task Scheduler Library
  • Zoek: WingetAutoUpdate_MicrosoftAzureStorageExplorer
  • Rechtermuisknop: Run (om handmatig te testen)

View last run result:
  Get-ScheduledTask -TaskName "WingetAutoUpdate_MicrosoftAzureStorageExplorer" | 
    Get-ScheduledTaskInfo | 
    Select LastRunTime, LastTaskResult

Manual update test:
  winget upgrade --id Microsoft.Azure.StorageExplorer --silent --scope machine
