...
Rule type:         Registry
Key path:          HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Logitech.Options
Value name:        DisplayName
Detection method:  String comparison
Operator:          Equals
Value:             Logitech Options

Alternative registry path (probeer beide):
HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Logitech.Options
...
Path:              C:\Program Files\Logitech\Logitech Options
File or folder:    Logitech Options.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor Logitech Options
$minVersion = [version]"9.70.68"

# Check via Registry
$regPaths = @(
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Logitech.Options",
    "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Logitech.Options"
)

foreach ($path in $regPaths) {
    $app = Get-ItemProperty -Path $path -ErrorAction SilentlyContinue | 
           Where-Object { $_.DisplayName -like "*Logitech Options*" }
    
    if ($app) {
        try {
            $installedVersion = [version]$app.DisplayVersion
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: Logitech Options $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: Logitech Options (version check failed)"
            exit 0
        }
    }
}
...
Package ID:  Logitech.Options
Source:      winget (Microsoft repository)

Test handmatig met:
winget search Logitech.Options
winget show Logitech.Options
...
Task Name:       WingetAutoUpdate_LogitechOptions
Trigger:         Dagelijks om 03:00 uur
Account:         SYSTEM (LocalSystem)
Privileges:      Hoogste niveau (Administrator)

Command:         winget upgrade --id Logitech.Options --silent --scope machine
...
Update logs:     %TEMP%\winget_update_LogitechOptions.log
PSADT logs:      C:\Windows\Logs\Software\Logitech Options\*.log

View scheduled task:
  • Open: Task Scheduler (taskschd.msc)
  • Navigeer: Task Scheduler Library
  • Zoek: WingetAutoUpdate_LogitechOptions
  • Rechtermuisknop: Run (om handmatig te testen)

View last run result:
  Get-ScheduledTask -TaskName "WingetAutoUpdate_LogitechOptions" | 
    Get-ScheduledTaskInfo | 
    Select LastRunTime, LastTaskResult

Manual update test:
  winget upgrade --id Logitech.Options --silent --scope machine
