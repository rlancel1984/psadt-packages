...
Rule type:         Registry
Key path:          HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\7zip.7zip
...
HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\7zip.7zip
...
Path:              C:\Program Files\Igor Pavlov\7-Zip
File or folder:    7-Zip.exe
...
────────────────────────────────────────────────────────────────────────
# Detection script voor 7-Zip
$minVersion = [version]"Latest"

# Check via Registry
$regPaths = @(
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\7zip.7zip",
    "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\7zip.7zip"
)
    ...
    if ($app) {
        try {
            ...
            if ($installedVersion -ge $minVersion) {
                Write-Output "Detected: 7-Zip $installedVersion"
                exit 0
            }
        } catch {
            Write-Output "Detected: 7-Zip (version check failed)"
            exit 0
        }
    }
}
...
Update logs:     %TEMP%\winget_update_7Zip.log
PSADT logs:      C:\Windows\Logs\Software\7-Zip\*.log

View scheduled task:
  • Open: Task Scheduler (taskschd.msc)
  • Navigeer: Task Scheduler Library
  • Zoek: WingetAutoUpdate_7Zip
  • Rechtermuisknop: Run (om handmatig te testen)

View last run result:
  Get-ScheduledTask -TaskName "WingetAutoUpdate_7Zip" | 
    Get-ScheduledTaskInfo | 
    Select LastRunTime, LastTaskResult

Manual update test:
  winget upgrade --id 7zip.7zip --silent --scope machine
